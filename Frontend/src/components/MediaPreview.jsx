...
function MediaPreview({ path, type, alt = 'תצוגת מדיה', className = '' }) {
    ...
    <div className={`media-preview ${className} ${loading ? 'is-loading' : ''}`}>
      ...
      {mediaType === 'video' ? (
        failed
          ...
          : <video src={url} controls playsInline preload="metadata" onLoadedData={() => setLoading(false)} onError={() => { setLoading(false); setFailed(true) }} />
      ) : (
